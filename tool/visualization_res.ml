...
let unflatten_exe = "/usr/local/bin/unflatten" ;;
let dot_exe = "/usr/local/bin/dot" ;;
...
let fault_dot ?(es=[]) lib model chan =
  ...
  let edges = List.filter gedges ~f:(fun x -> (not (List.mem es x)))  in 
...
let rec cons_fault_edges library instances connections nodes his fl res =
  match fl with
	...
	library instances connections nodes f fmx in
      let nedges = List.filter edges ~f:(fun x -> not (List.mem res x)) in
      let nfaults = List.filter nextfaults
	~f:(fun x -> not (List.mem his x)) in
      cons_fault_edges
...
let find_res_port nodes top =  (* (node id, port id) *)
  ...
  let (inst, node_id, inputs, outputs) =
  List.find_exn nodes
  ~f:(fun x -> let (inst, _, _, _) = x in
