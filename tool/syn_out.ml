...
(* updates: *)
(* 1/22/2018 - KS: first upload to GitHub repo *)
...
(*Assumes the following is run first
 * 1: fault-tree.ml
 * 2: val-checks.ml
 * 3: syn-algo-rev2.ml
 * 4: synval.ml
 * 5: syn-navpos-rev2.ml (example) 
 *)
...
(* Another feature - Architecture Catalog
 * 10: Run "dot_gen_show_funct_file" for each architecture
 *)

(* Temp forcing based on capability of example*)
(*let user_probTargets = 
  [
   {target = "FMA"; prob = ("loa", 5e-05)};
...
(* Works for Printing Good/Bad for each Arch
 **********************************
let numTars = List.length user_probTargets in
let numArch = (Hashtbl.length myh_sol_prob) /2 in

for ar = 1 to numArch do
        printf "\nArch #%i: " ar;
        for t = 1 to numTars do (* for each target *)
                let curTar = List.nth_exn user_probTargets (t-1) in
                let (tarflt, tarprob) = curTar.prob in
                let value = Hashtbl.find_exn myh_sol_prob (ar,tarflt) in
                let (_,sol) = value in
                if sol <= tarprob then
                        begin
                        (*Arch Good*)
                        (ar,tarflt) :: goodarchs2;
                        printf "F#%i - Good " t
                        end
                else printf    "F#%i - Bad  " t
...
(* New Hash for each Arch with additional data
 * Key: Arch#
...
let syn_results user_pt arch_res_h sol_h sort_by =
        ...
        let res_list = Hashtbl.fold ~f:(fun ~key:_ ~data:d acc -> d::acc) 
                allgood_h ~init:[] in
        let sorted_res_ll = List.sort ~cmp:(fun x y -> sorter x y sort_by) res_list in
