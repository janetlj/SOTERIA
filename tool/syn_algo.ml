...
let rec generate_archParamListList subarchL hf hc hf2cn =
  ...
  | hd::tl ->
    ...
    let source_map = Map.of_alist_exn source_icnL ~comparator:Int.comparator in
    ...
    | thd::ttl ->
      match foutl with
        ...
        | fhd::ftl ->
          ...
          if (List.equal t f ~equal:(fun x y -> x=y))
...
let generate_voteConnects cl voterComp myh_f2s_out myh_s2f_out =
  ...
  let lc_to_inp_converted = List.map ~f:(fun x -> let (cn,iflow) = x in let inp = List.Assoc.find_exn voterComp.input_metad iflow in (cn,inp)) lc_to_inp in
  ...
  (List.map c ~f:(fun x -> let ((i_name, i_src),(o_name, o_src)) = x 
             in ((i_name,List.Assoc.find_exn rev_inp_md i_src),
