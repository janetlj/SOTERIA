(* File: navigation_position.ml *)
(* Date: 1/22/2018 *)
(* Use with architecture synthesis algorithm, syn_algo.ml *)

(* updates: *)
(* 1/22/2018 - KS: first upload to repo. *)
...
let xfmagen cn fl al =
  ...
  and replace_mdata x = List.map ~f:(fun z -> 
    let (c, _) = List.find_exn
      ~f:(fun (a,b) -> List.equal b z String.equal) input_metad in c)
    x in
